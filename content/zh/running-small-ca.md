...
颁发机构生成这一持有者（让我们把他称为「张三」）的证书文件 `user1.crt.pem` 后，需要把证书文件发回给张三，然后张三在进行网络通信的时候，就需要给通信的对端（这里我们称其为「李四」）提供自己的证书信息。为了验证证书确实是张三的，李四需要事先获取 Swan CA （签名）验证过的根证书 ca.crt.pem ，在通信开始前配置好软件工具指向证书数据文件，用颁发机构的证书就可以验证通信过程中收到的某一证书确实是张三的。
